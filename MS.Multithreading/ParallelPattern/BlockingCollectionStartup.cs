...
namespace MS.Multithreading.ParallelPattern
{
    public class BlockingCollectionStartup
    {
        ...
        public static void Startup()
        {
            ...
            var filter2 = new PipelineWorker<decimal, string>(filter1.Output, n => string.Format("--{0}--", n), cts.Token, "filter2");

            var filter3 = new PipelineWorker<string, string>(filter2.Output, (s) => Console.WriteLine("The final results is {0} on thread id {1}", s, Thread.CurrentThread.ManagedThreadId), cts.Token, "filter3");
